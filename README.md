...
1. **Real-time hourly climate data** directly from the **NASA POWER API** (Surface temperature $T_{2M}$, relative humidity $RH_{2M}$, wind speed $WS_{10M}$, and downward solar irradiance $ALLSKY\_SFC\_SW\_DWN$).
...
- **Backend**:
  ...
  - Axios (NASA POWER API communication)
...
│   │   │   └── weatherRoutes.js   # GET /api/weather (NASA POWER)
│   │   ├── services/
│   │   │   ├── materialService.js
...
## 6. NASA POWER API Integration Details

The software connects to NASA's official Langley Research Center POWER API:
...
- **Data Policy**: **Strictly Real Observations**. The system rejects incomplete records and will never fabricate synthetic climate data in production.
