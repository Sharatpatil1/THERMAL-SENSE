...
export default function ThermalResults({
  ...
  const handleRunSimulation = async () => {
    if (!hasClimate) {
      setError('Live weather data is required. Please refresh live weather data first.');
      return;
    }
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      const payload = {
        ...
        weatherSource: weatherData.source || 'Open-Meteo Live Weather',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {!hasClimate && (
        <div className="alert alert-warning">
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700 }}>Live weather data required</div>
            <div style={{ fontSize: '0.82rem', marginTop: '0.2rem' }}>
              Simulation is locked until real current weather and 24-hour hourly environmental data are loaded.
            </div>
          </div>
          <button
            type="button"
